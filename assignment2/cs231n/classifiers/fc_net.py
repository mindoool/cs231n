...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # scores
        z_relu, z_relu_cache = affine_relu_forward(X, W1, b1) 
        scores, scores_cache = affine_forward(z_relu, W2, b2)
        ...
        # dz, dW2, db2
        dz, dW2, db2 = affine_backward(dL, scores_cache)
        dW2 += reg * W2

        # dx, dW1, db1
        dx, dW1, db1 = affine_relu_backward(dz, z_relu_cache)
        dW1 += reg * W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        self.dims_list = list(hidden_dims)
        self.dims_list.insert(0, input_dim)
        self.dims_list.append(num_classes)
        self.len_dims_list = len(self.dims_list)
        dims_list = self.dims_list
        for i in range(self.len_dims_list-1):
            self.params["W"+str(i+1)] = np.random.normal(0, weight_scale, (dims_list[i], dims_list[i+1]))
